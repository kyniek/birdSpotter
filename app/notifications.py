...
from .config import settings, get_smtp_user, get_smtp_password
...
def send_notification(
    ...
    try:
        with smtplib.SMTP(settings.smtp_host, settings.smtp_port) as server:
            server.starttls()
            server.login(get_smtp_user(), get_smtp_password())
            server.sendmail(get_smtp_user(), coordinators, msg.as_string())
    except Exception:
